...
const RegisterRefugioSchema = z.object({
  email: 
...
export default async function RegisterRefugio(email:string,password:string,confirmPassword:string,name:string): Promise<ResponseType<void>>{
    try{
        const parsedData = await parseSchema(RegisterRefugioSchema, {email,password,confirmPassword,name});
        const useCase = new RegisterRefugioUseCase();
