...
import { StartDisableTwoFactorUseCase } from "@/backend/context/Auth/app/StartDisableTwoFactorUseCase";
...
export type DisableTwoFactorStartData = {
  setupTicket: string;
};

export default async function StartDisableTwoFactorAction(): Promise<
  ResponseType<DisableTwoFactorStartData>
> {
  try {
    ...
    const useCase = new StartDisableTwoFactorUseCase(
      authRepository,
      otpRepository,
      sendLoginOtpEmail
    );
